    ...
    df %<>% select(FY, org_grp, x, y, z)
      ...
      df %<>%
        filter(ServiceType %in% servicetype_filt
               ...
               & Code_Mod %in% code_filt
               & org_grp %in% org_filt) %>%
        group_by(FY,org_grp,ServiceType) %>%
        summarize(
          SumOfCases = sum(SumOfCases, na.rm = T),
          SumOfUnits = sum(SumOfUnits, na.rm = T),
          SumOfCost = sum(SumOfCost, na.rm = T),
      ...
      df %<>% select(FY, org_grp, ServiceType, x, y, z)
    }
  ...
  output$bubble <- renderPlotly({
    ...
    if(input$select_org == "All") {
      ...
      df_bubble() %>%
        ...
        layout(
          title = ~paste('How does',input$x,'compare to',input$y,'for<br>',
                         input$select_ServiceType,'across',input$select_org,'?'),
          xaxis = list(
            title = input$x,
            range = c(0, max_x),
            showgrid = FALSE
      ...
      df_bubble() %>%
        ...
        plot_ly(
          x = ~x, y = ~y, type = 'scatter', mode = 'markers', 
          size = ~z, color = ~ServiceType, colors = cmh_palette, marker = list(opacity = 0.5),
          hoverinfo = 'text',
          text = ~paste(
            ServiceType,
            '<br>',input$x,':',
        ...
        layout(
          title = ~paste('How does',input$x,'compare to',input$y,
                         '<br>by service for',input$select_org,'?'),
          xaxis = list(
            title = input$x,
            range = c(0, max_x),
            showgrid = FALSE
