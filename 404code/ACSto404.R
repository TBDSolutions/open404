...
# Define PIHP region 5
# lookup_MI5 <- geo.lookup(state=c("MI"), 
#                          county=c("Bay","Arenac","Clare","Gladwin","Isabella",
#                                   "Mecosta","Midland","Osceola","Clinton",
#                                   "Eaton","Ingham","Gratiot","Huron","Ionia",
#                                   "Jackson","Hillsdale","Montcalm","Newaygo",
#                                   "Saginaw","Shiawassee","Tuscola"))
#####

# Get 5-year estimate data for population
#####
MI_2012 <- acs.fetch(endyear = 2012, 
...
MI_df <- rbind(MI_df_12,MI_df_11, MI_df_10) #bind the 3 years together
...
#####
# Read in 404 data
subMasterPlus <- read.csv('https://raw.githubusercontent.com/j-hagedorn/open404/master/data/clean/subMasterPlus', sep=',', header=TRUE)
FY10to12 <- subset(subMasterPlus, FY == 2010 | FY == 2011 | FY == 2012 )
...
# Create new variable, Cost Per Member Per Month
merged$CostPMPM <- round((merged$SumOfCost/merged$SumOfCases)/12,digits = 2)
...
merged <- subset(merged, select=c(FY,PIHPname,PIHP,CMHSP,TotalPop,
                        ...
                        CostPerCase,CostPMPM,CostPerUnit,UnitPerCase,
                        Serv_Perc_Tot,Cost_Perc_Tot,
                        Cost1kPop,Srvd1kPop))
# rename
merged10to12 <- merged

# Output merged10to12 .csv file
write.csv(merged10to12, 
          file="C:\\Users\\Josh\\Documents\\GitHub\\open404\\data\\clean\\merged10to12")
#####
